...
`bin/omarchy-scripts` locates the repository-relative `src/`, `lib/`, and `scripts/` directories, then launches the CLI. The CLI is a thin JSON interface over the engine: `list`, `info`, `run`, `last-run`, `edit`, `new`, `copy`, `delete`, `validate`, and `config` (`init`/`list-dirs`/`add-dir`/`remove-dir` plus generic `get`/`set`/`unset`). `edit` replaces the CLI process with `$EDITOR` (falling back to `vi`); the QML frontend uses Omarchy's editor launcher for the same file-oriented action. `copy <id> <new-id>` duplicates an existing script's file into the workspace under a new id — validated the same kebab-case shape and uniqueness discovery already enforces — rewriting only its `@script.id` (and `@script.title`, appending " (copy)") lines; it is the only way to legitimately clone a script's file without it colliding with the original's id.
